...
class PatientProfile(db.Model):
    ...
    def serialize_full(self):
        ...
        serialized_data.update({
            ...
            # **DIUBAH**: Menggunakan helper untuk mendapatkan URL dari Azure
...
class Gerakan(db.Model):
    ...
    # Nama kolom diubah agar lebih jelas bahwa ini adalah path blob
    ...
    def serialize_full(self): # **DIUBAH**: Tidak perlu parameter `app_config`
        pembuat_info = self.pembuat.serialize_basic() if self.pembuat else None
        return {
            "id": self.id,
            "nama_gerakan": self.nama_gerakan,
            "deskripsi": self.deskripsi,
            # **DIUBAH**: Langsung panggil helper get_blob_url
...
# --- Model ProgramRehabilitasi & lainnya (Hanya perubahan kecil) ---
class ProgramRehabilitasi(db.Model):
    ...
    def serialize_full(self): # **DIUBAH**: Tidak perlu `app_config`
        ...
        for detail in self.detail_gerakan.order_by(ProgramGerakanDetail.urutan.asc(), ProgramGerakanDetail.id.asc()).all():
            ...
            if gerakan_obj:
                gerakan_data = gerakan_obj.serialize_full() # **DIUBAH**
        ...
        return {
            ...
            "pasien": pasien_info,
            "list_gerakan_direncanakan": list_gerakan_details,
            "created_at": self.created_at.isoformat() if self.created_at else None,
            "updated_at": self.updated_at.isoformat() if self.updated_at else None,
        }
...
class LaporanRehabilitasi(db.Model):
    ...
    def serialize_full(self): # **DIUBAH**: Tidak perlu `app_config`
        ...
        for detail_hasil in self.detail_hasil_gerakan.order_by(LaporanGerakanHasil.urutan_gerakan_dalam_program.asc(), LaporanGerakanHasil.id.asc()).all():
            detail_gerakan_list.append(detail_hasil.serialize()) # **DIUBAH**
        # ... sisa fungsi sama ...
        ...
        nama_terapis_program = self.program_asli.terapis.nama_lengkap if self.program_asli and self.program_asli.terapis else "N/A"
...
class LaporanGerakanHasil(db.Model):
    ...
    def serialize(self): # **DIUBAH**: Tidak perlu `app_config`
        gerakan_info = self.gerakan_asli.serialize_full() if self.gerakan_asli else {} # **DIUBAH**
        return {
            ...
            "jumlah_sempurna": self.jumlah_sempurna, "jumlah_tidak_sempurna": self.jumlah_tidak_sempurna, "jumlah_tidak_terdeteksi": self.jumlah_tidak_terdeteksi,
